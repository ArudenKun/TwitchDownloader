...
public abstract class AvaloniaHostingApplication<TMainWindow> : Application, IAvaloniaHostingApplicationInitializer
    where TMainWindow : Window
{
    ...
    {
        _mainWindowKey = $"{_random.Next()}-{typeof(TMainWindow).FullName}";
    }
    ...
    public new IClassicDesktopStyleApplicationLifetime ApplicationLifetime =>
        ...
        throw new InvalidOperationException("hosting only supports desktop lifetime or lifetime is not set");

    void IAvaloniaHostingApplicationInitializer.InitializeHost(Action<IServiceCollection>? configureServices)
    {
    ...
    private void ConfigureRequiredServices(IServiceCollection services)
    {
        ...
        services.AddSingleton<TopLevel>(sp => sp.GetRequiredKeyedService<TMainWindow>(_mainWindowKey));
    ...
    protected virtual void OnInitialized(IServiceProvider services)
    {
    }

    protected virtual Task ConfigureMainWindow(TMainWindow mainWindow, IServiceProvider services) => Task.CompletedTask;
